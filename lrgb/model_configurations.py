...
from function_drew_gnn import DRewGNNStage

# change opt structure to cfg structure
...
def set_block(cfg):
    ...
    if ode_str == 'constant_frac':
        ...
        raise BlockNotDefined(f"Block type '{ode_str}' not defined.")
    return block


def set_function(cfg):
    ode_str = cfg.model.function
    if ode_str == 'laplacian':
        ...
        raise FunctionNotDefined(f"Function type '{ode_str}' not defined.")
    return f
