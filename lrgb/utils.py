...
def coo2tensor(coo, device=None):
    ...
    shape = coo.shape
    print(f'adjacency matrix generated with shape {shape}')
    return torch.sparse.FloatTensor(i, v, torch.Size(shape)).to(device)


def get_sym_adj(data, cfg, improved=False):
    edge_index, edge_weight = gcn_norm(
        data.edge_index, data.edge_attr, data.num_nodes,
        improved, cfg.model.self_loop_weight > 0, dtype=data.x.dtype)
    coo = to_scipy_sparse_matrix(edge_index, edge_weight)
    return coo2tensor(coo)


def get_rw_adj_old(data, cfg):
    if cfg.model.self_loop_weight > 0:
        edge_index, edge_weight = add_remaining_self_loops(
            data.edge_index, data.edge_attr, fill_value=cfg.model.self_loop_weight)
    else:
...
def get_rw_adj(edge_index, edge_weight=None, norm_dim=1, fill_value=0., num_nodes=None, dtype=None):
    ...
    deg_inv = deg.pow_(-1)
    edge_weight = deg_inv[indices] * edge_weight if norm_dim == 0 else edge_weight * deg_inv[indices]
